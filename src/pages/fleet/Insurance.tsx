...
const STORAGE_KEY = "fleet_insurance";
...
export default function Insurance() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-3 md:p-6">
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">Fleet Management › Insurance Management</p>
          </div>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    {["VEHICLE", "POLICY NO", "PROVIDER", "COVERAGE", "EXPIRY", "STATUS", "ACTIONS"].map((h) => (
                  ...
                  {filtered.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-xl bg-red-50 text-red-500 flex items-center justify-center font-bold text-sm border border-red-100 group-hover:scale-110 transition-transform">
                            {r.vehicleNo.charAt(0)}
                          </div>
                          <span className="font-bold text-slate-900">{r.vehicleNo}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-mono text-slate-600">{r.policyNo}</td>
                      <td className="px-6 py-4 font-medium text-slate-600">{r.provider}</td>
                      <td className="px-6 py-4 text-slate-600">{r.coverage}</td>
                      <td className="px-6 py-4 font-medium text-slate-600">{r.expiryDate}</td>
                      <td className="px-6 py-4">
              ...
              <div key={r.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{r.policyNo}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-slate-400">Coverage</span><span className="font-bold">{r.coverage}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Expiry</span><span className="font-bold">{r.expiryDate}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Premium</span><span className="font-bold text-green-600">{r.premium}</span></div>
                </div>
      ...
      {viewRecord && (
        <Modal onClose={() => setViewRecord(null)} title="Policy Details">
          <div className="space-y-4">
            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">{viewRecord.policyNo}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(viewRecord).map(([k, v]) =>
                k !== "id" && k !== "vehicleNo" && k !== "policyNo" ? (
...
function FormModal({ editData, onClose, onSave }: { editData: Insurance | null; onClose: () => void; onSave: (data: Insurance) => void }) {
  const [form, setForm] = useState<Insurance>(
    editData || { id: 0, vehicleNo: "", policyNo: "", provider: "", expiryDate: "", coverage: "Comprehensive", premium: "", status: "Active" }
  );
  ...
  const validate = () => {
    ...
    if (!form.policyNo.trim()) newErrors.policyNo = "Required";
  ...
  return (
    <Modal onClose={onClose} title={editData ? "Edit Policy" : "Add Policy"}>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <FormInput label="Vehicle No" value={form.vehicleNo} onChange={(v) => setForm({ ...form, vehicleNo: v })} error={errors.vehicleNo} />
          <FormInput label="Policy No" value={form.policyNo} onChange={(v) => setForm({ ...form, policyNo: v })} error={errors.policyNo} />
        </div>
        <FormInput label="Provider" value={form.provider} onChange={(v) => setForm({ ...form, provider: v })} error={errors.provider} fullWidth />
        <div className="grid grid-cols-2 gap-4">
          <FormInput label="Coverage" value={form.coverage} onChange={(v) => setForm({ ...form, coverage: v })} />
          <FormInput label="Premium" value={form.premium} onChange={(v) => setForm({ ...form, premium: v })} placeholder="₹" />
        </div>
        <FormInput label="Expiry Date" value={form.expiryDate} onChange={(v) => setForm({ ...form, expiryDate: v })} placeholder="DD-MM-YYYY" fullWidth />
