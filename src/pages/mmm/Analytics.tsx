    ...
    <div className="min-h-screen bg-[#F1F5F9] p-4 sm:p-6 lg:p-10 animate-in fade-in duration-700">
      <div className="max-w-[1600px] mx-auto space-y-10">
              ...
              <div>
                <div className="flex items-center gap-3">
                  <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">
                    Marketing <span className="text-red-600">Analytics</span>
                  </h1>
                </div>
                <p className="text-slate-500 mt-2 font-semibold text-lg">
                  MMM › Data-driven insights for your marketing performance
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { label: "Click Through Rate", value: "3.8%", icon: TrendingUp, color: "blue" },
            { label: "Conversion Value", value: "$85,400", icon: BarChart3, color: "emerald" },
            { label: "Engagement Score", value: "78/100", icon: PieChart, color: "rose" },
          ].map((stat, i) => (
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-[40px] p-8 shadow-2xl shadow-slate-200/50 min-h-[400px] flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-2xl font-black text-slate-900">Insufficient Data</h3>
          <p className="text-slate-500 mt-2 font-semibold">Keep running campaigns to see advanced analytics</p>
        </div>
      </div>
    </div>
  );
